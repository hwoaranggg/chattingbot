...
from services.payment_service import (
    create_yukassa_payment, create_sbp_payment,
    create_crypto_invoice, check_yukassa_payment
)
from services.database import set_paid, save_payment
...
async def pay_yukassa(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Нажми кнопку, оплати и вернись — нажми «Я оплатил».\n\n"
...
async def pay_sbp(callback: CallbackQuery):
    ...
    await callback.answer()
    await callback.message.edit_text("⏳ Создаём счёт...")
    data = await create_sbp_payment(tg_id)
    await callback.message.edit_text(
        f"🏦 <b>Оплата через СБП</b>\n\n"
        f"Нажми кнопку, оплати через приложение банка и вернись — нажми «Я оплатил».\n\n"
        f'<a href="{data["confirmation_url"]}">👉 Перейти к оплате</a>',
        reply_markup=check_payment_keyboard("yukassa", data["payment_id"]),
        parse_mode="HTML"
    )
...
async def check_payment(callback: CallbackQuery, bot: Bot):
    ...
    if provider == "yukassa":
        ...
        paid = await check_crypto_invoice(payment_id)
